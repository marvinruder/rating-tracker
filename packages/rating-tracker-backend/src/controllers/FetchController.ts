    ...
    for await (const stock of stocks) {
      if (
        ...
        new Date().getTime() - stock.morningstarLastFetch.getTime() <
          1000 * 60 * 60 * 12 // We only fetch stocks that have not been fetched in the last 12 hours.
      ) {
        logger.info(
          PREFIX_CHROME +
            `Stock ${
      ...
      try {
        ...
        try {
          const dividendYieldPercentString = await driver
            ...
            .getText(); // Example: "2.1", or "-" if there is no dividend yield.
        ...
        try {
          ...
          ).replaceAll(",", ""); // Example: "20.5", "1,000" for larger numbers, or "-" if there is no P/E.
        ...
        try {
          let currencyString = await driver
            ...
            .getText(); // Example: "17:35:38 CET | EUR  Minimum 15 Minutes Delay."
        ...
        try {
          ...
          ).replaceAll(",", ""); // Example: "1,000.00", or "-" if there is no last close.
          if (lastCloseString === "-") {
            lastClose = null;
          } else {
            if (
    ...
    for await (const stock of stocks) {
      if (
        ...
        new Date().getTime() - stock.msciLastFetch.getTime() <
          1000 * 60 * 60 * 24 * 14 // We only fetch stocks that have not been fetched in the last 14 days.
      ) {
        logger.info(
          PREFIX_CHROME +
            `Stock ${
    ...
    for await (const stock of stocks) {
      if (
        ...
        new Date().getTime() - stock.refinitivLastFetch.getTime() <
          1000 * 60 * 60 * 24 * 7 // We only fetch stocks that have not been fetched in the last 7 days.
      ) {
        logger.info(
          PREFIX_CHROME +
            `Stock ${
