...
tests.push({
  ...
  testFunction: async () => {
    const res = await supertest.head(`${baseURL}${sessionEndpointPath}`).set("Cookie", ["authToken=exampleSessionID"]);
...
tests.push({
  ...
  testFunction: async () => {
    const res = await supertest.head(`${baseURL}${sessionEndpointPath}`).set("Cookie", ["authToken=invalidSessionID"]);
...
tests.push({
  ...
  testFunction: async () => {
    let res = await supertest.delete(`${baseURL}${sessionEndpointPath}`).set("Cookie", ["authToken=exampleSessionID"]);
    expect(res.status).toBe(204);
