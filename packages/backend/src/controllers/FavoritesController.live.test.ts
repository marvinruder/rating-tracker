...
tests.push({
  ...
  testFunction: async () => {
    const res = await supertest.get(`${baseURL}${favoritesEndpointPath}`).set("Cookie", ["authToken=exampleSessionID"]);
...
tests.push({
  ...
  testFunction: async () => {
    ...
    res = await supertest
      .get(`${baseURL}${favoritesEndpointPath}`)
      .set("Cookie", ["authToken=anotherExampleSessionID"]);
    expect(res.status).toBe(200);
    expect(res.body.name).toBe("Favorites");
...
tests.push({
  ...
  testFunction: async () => {
    let res = await supertest
      .put(`${baseURL}${favoritesEndpointPath}/exampleMELI`)
      .set("Cookie", ["authToken=anotherExampleSessionID"]);
    ...
    res = await supertest
      .put(`${baseURL}${favoritesEndpointPath}/doesNotExist`)
      .set("Cookie", ["authToken=anotherExampleSessionID"]);
...
tests.push({
  ...
  testFunction: async () => {
    let res = await supertest
      .delete(`${baseURL}${favoritesEndpointPath}/exampleAAPL`)
      .set("Cookie", ["authToken=exampleSessionID"]);
    expect(res.status).toBe(204);

    res = await supertest.get(`${baseURL}${favoritesEndpointPath}`).set("Cookie", ["authToken=exampleSessionID"]);
    expect(res.status).toBe(200);
    expect(res.body.name).toBe("Favorites");
    expect(res.body.stocks.length).toBe(1);
    ...
    res = await supertest
      .delete(`${baseURL}${favoritesEndpointPath}/doesNotExist`)
      .set("Cookie", ["authToken=exampleSessionID"]);
    expect(res.status).toBe(404);
  },
});
