...
tests.push({
  ...
  testFunction: async () => {
    const res = await supertest.get(`${baseURL}${favoritesAPIPath}`).set("Cookie", ["id=exampleSessionID"]);
...
tests.push({
  ...
  testFunction: async () => {
    ...
    let res = await supertest.get(`${baseURL}${watchlistsAPIPath}`).set("Cookie", ["id=anotherExampleSessionID"]);
    expect(res.status).toBe(200);
    expect(res.body.length).toBe(0);

    // Read the Favorites list (since there is one, a new and empty one will be created)
    res = await supertest.get(`${baseURL}${favoritesAPIPath}`).set("Cookie", ["id=anotherExampleSessionID"]);
    expect(res.status).toBe(200);
    expect(res.body.name).toBe("Favorites");
    expect(res.body.stocks.length).toBe(0);

    // Check that the Favorites list is still around
    res = await supertest.get(`${baseURL}${watchlistsAPIPath}`).set("Cookie", ["id=anotherExampleSessionID"]);
    expect(res.status).toBe(200);
...
tests.push({
  ...
  testFunction: async () => {
    ...
    res = await supertest.get(`${baseURL}${favoritesAPIPath}`).set("Cookie", ["id=exampleSessionID"]);
    expect(res.status).toBe(200);
    expect(res.body.name).toBe("Favorites");
    expect(res.body.stocks.length).toBe(1);
    ...
    res = await supertest.delete(`${baseURL}${favoritesAPIPath}/exampleAAPL`).set("Cookie", ["id=exampleSessionID"]);
    expect(res.status).toBe(204);
    ...
    res = await supertest.delete(`${baseURL}${favoritesAPIPath}/doesNotExist`).set("Cookie", ["id=exampleSessionID"]);
    expect(res.status).toBe(404);
  },
});
