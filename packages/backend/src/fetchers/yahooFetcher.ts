 ...
 */
const yahooFetcher: Fetcher = async (req: Request, stock: Stock): Promise<void> => {
  ...
  const json = await getJSON(`https://query1.finance.yahoo.com/v8/finance/chart/${stock.ticker}`, {
    params: {
  ...
  try {
    ...
    if (stock.currency !== null) {
      // If a currency is already stored in the database, but we cannot extract it from the JSON object, we log this as
  ...
  try {
    ...
    prices1y = adjclose.reduce((acc, val, i, arr) => ((arr.length - i) % 5 === 1 ? [...acc, val] : acc), []);
  } catch (e) {
    ...
    if (
      ...
      stock.low52w !== null ||
      stock.high52w !== null ||
      stock.prices1y?.length > 0 ||
      stock.prices1mo?.length > 0
    ) {
      ...
      errorMessage += `\n\tUnable to extract prices: ${String(e.message).split(/[\n:{]/)[0]}`;
    }
  }
