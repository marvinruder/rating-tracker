...
FROM --platform=$BUILDPLATFORM node:22.8.0-alpine AS yarn
...
FROM --platform=$BUILDPLATFORM node:22.8.0-alpine AS test-backend
ENV FORCE_COLOR=true
...
FROM --platform=$BUILDPLATFORM node:22.8.0-alpine AS test-commons
ENV FORCE_COLOR=true
...
FROM --platform=$BUILDPLATFORM node:22.8.0-alpine AS test-frontend
ENV FORCE_COLOR=true
...
FROM --platform=$BUILDPLATFORM node:22.8.0-alpine AS build-backend
...
FROM --platform=$BUILDPLATFORM node:22.8.0-alpine AS build-frontend
ENV NODE_ENV=production
ENV FORCE_COLOR=true
...
FROM node:22.8.0-alpine AS node
