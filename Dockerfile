...
FROM --platform=$BUILDPLATFORM rust:1.87.0-alpine AS wasm
...
FROM --platform=$BUILDPLATFORM node:22.15.1-alpine AS yarn
...
FROM --platform=$BUILDPLATFORM node:22.15.1-alpine AS test-backend
ENV FORCE_COLOR=true
...
FROM --platform=$BUILDPLATFORM node:22.15.1-alpine AS test-commons
ENV FORCE_COLOR=true
...
FROM --platform=$BUILDPLATFORM node:22.15.1-alpine AS test-frontend
ENV FORCE_COLOR=true
...
FROM --platform=$BUILDPLATFORM node:22.15.1-alpine AS build-backend
...
FROM --platform=$BUILDPLATFORM node:22.15.1-alpine AS build-frontend
ENV NODE_ENV=production
ENV FORCE_COLOR=true
...
FROM node:22.15.1-alpine AS node
